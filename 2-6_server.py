# Clone blocks and Broadcast between nodes
...
class BlockChain:
	...
	## ------------------------ 2-6 starts from ------------------------>>>>>>
		...
		# For broadcast block
		self.received_verified_block = False
	## <<<<<<------------------------ 2-6 ends ------------------------
	...
	#------------ Initialize a transaction (Client side) ------------
	...
	## ------------------------ 2-6 starts from ------------------------>>>>>>
		...
		while new_block.hash[0: self.difficulty] != '0' * self.difficulty:
			new_block.nonce += 1
			new_block.hash = self.get_hash(new_block, new_block.nonce)
			if self.received_verified_block:
		...
		with connection:
			...
			while True:
				...
				## ------------------------ 2-6 starts from ------------------------>>>>>>
					...
					## Receive a request of cloning blocks 
					...
					## Receive a request of adding new nodes
...
## ------------------------ 2-6 starts from ------------------------>>>>>>
	...
	def clone_blockchain(self, address):
		...
		while True:
			response += client.recv(4096) ## there is a risk due to using 4096
	...
	#------------ Broadcast new block / new transaction to other nodes ------------
	...
	def receive_broadcast_block(self, block_data):
		...
		if block_data.previous_hash != last_block.hash:
			...
			if block_data.hash[0: self.difficulty] == '0' * self.difficulty:
				...
				self.received_verified_block = True
